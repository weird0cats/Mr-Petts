...
# Pet commands
@bot.slash_command(
    description="Claim yourself a pet! doesn't work if there is an active pet."
)
async def adopt(interaction: Interaction, name: str):
    try:
        jsonfs.create(f"{interaction.user.id}.jsonc", {"name": name, "love": 0})
        await interaction.send(
            f"""Congratulations, {interaction.user.name}, you have adopted {name}!""",
            ephemeral=True,
        )
...
async def play(interaction: Interaction):
    try:
        ...
        data["love"] += 1
        await interaction.send(
            f"You spent some time playing with {data['name']}.", ephemeral=True
        )
        jsonfs.write(f"{interaction.user.id}.jsonc", data)
    except FileNotFoundError:
        await interaction.send(
            "You don't have a pet! you can adopt one with /adopt!", ephemeral=True
        )
...
async def play_error(interaction: Interaction, error):
    if isinstance(error, cooldowns.exceptions.CallableOnCooldown):
        try:
            await interaction.send(
                f"{jsonfs.read(f'{interaction.user.id}.jsonc')["name"]} is tired, wait a little bit!",
                ephemeral=True,
            )
        except FileNotFoundError:
            await interaction.send(
                f"Why are you trying to play with a pet you don't have, {interaction.user.name}?",
                ephemeral=True,
            )
